...
out3 = " 1. Daniel Ricciardo     | RED BULL RACING TAG HEUER | 0:57:12.013\n"
cases = [
    (['--files', '../tests/reports'], out),
    (['--files', '../tests/reports', '--desc'], out2),
    (['--files', '../tests/reports', 'driver', 'Daniel Ricciardo'], out3),
]


@pytest.mark.parametrize("params,expected", cases)
def test_CLReportMonaco_success(capfd, params, expected):
    CLReportMonaco().run(params)
    actual, err = capfd.readouterr()
    assert actual == expected


def test_CLReportMonaco_file_exception():
    with pytest.raises(FileNotFoundError):
        CLReportMonaco().run(['--files', 'files wrong_path_to_file'])


def test_CLReportMonaco_no_results(capfd):
    expected = "No results\n"
    CLReportMonaco().run(
        ['--files', '../tests/reports', 'driver', 'Wrong Name'])
    actual, err = capfd.readouterr()
    assert actual == expected
