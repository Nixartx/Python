...
def add_student_to_db(f_name,l_name):
    ...
    st_ins = students.insert().values(first_name=f_name, last_name=l_name).returning(students.columns.id)
    con = engine.connect()
    st_id = None
...
def find_schedule_first():
    engine = create_engine(testdb_path)
    metadata = MetaData()
    schedules = Table('schedule', metadata, autoload=True, autoload_with=engine)
    schedule = schedules.select()
    con = engine.connect()
    result = None
    for row in con.execute(schedule):
        result = {'student_id':row['student_id'], 'course_id':row['course_id']}
        break
    return result
...
def test_delete_course_to_student(client):
    ...
    result = client.delete('/student/{}/course/{}'.format(schedule['student_id'],schedule['course_id']))
    assert result.status_code == 204
